...
function App() {
  ...
  useEffect(() => { api("/api/auth/me").then(({ user }) => setUser(user)).catch(() => setUser(null)).finally(() => setRestoringSession(false)); }, []);
  ...
  if (restoringSession) return <main aria-busy="true" aria-label="Restoring session" className="grid min-h-screen place-items-center bg-[#071018] p-6 text-slate-100"><div className="grid place-items-center gap-4"><span className="grid size-12 place-items-center rounded-full border border-cyan-200/40 bg-cyan-200/10 text-2xl text-cyan-100">◌</span><p className="font-mono text-xs tracking-[0.18em] text-cyan-100">RESTORING ATLAS SESSION</p></div></main>;
